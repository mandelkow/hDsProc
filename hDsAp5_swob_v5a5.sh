#!/bin/bash -e
# hDsAp5_swob_v5a5.sh [++++] Process Dante's Sleep1 fMRI dataset.
...
MinTR=600 # min. exp. length 30 minutes
# SLORD="alt+z2"
...
-s | --sub          #  subject ID
-W | --rwin         #  phys.regression window + step size see hmat2txt
...
if [[ $MODE == "Swarm" ]]; then
    ...
    cd $InDir
    echo $PWD
    find -L $SUB -maxdepth 1 -type d -name "ses-*" -exec mkdir -p $OutDir/{} \;
    find -L $SUB -maxdepth 1 -type d -name "ses-*" -exec ln -s $PWD/{} $OutDir/{}/raw \;
    cd $OutDir
...
if [[ $JOB == *"Ric"* ]]; then
    ...
    if [[ $JOB == *"Ricct"* ]]; then
        ...
        hmat2txt.py -f -D -R $NTR -W $RWIN "${ExId}_phy.mat" "CardRegHil,RespRegHil" > RicRegs.1D # *** Resp+Card! NOT TESTED!
...
### Create censoring file RicRegs_censor.1D from resp/card mask for use with -regress_censor_extern
    ...
    hmat2txt.py -f "${ExId}_phy.mat" "RespRegMask" RicRegMask.txt
    hmat2txt.py -f "${ExId}_phy.mat" "CardRegMask,RespRegMask" RicRegMask.txt
...
if [[ $JOB == *"Reg"* ]]; then 
    ...
    if [[ -f RicRegs_censor.1D ]] && (( `grep -c 0 RicRegs_censor.1D` > `grep -c 1 RicRegs_censor.1D` )) ; then
        echo "Oops! Censored volumes should be 0 (zero)!!!"
        exit 1
    fi
...
echo DONE. COMPLETED: $0 # Ensure no exit 1 from above.
